...
# 复杂性子策略回测（每个子策略都支持 --backtest-start-date）
./scripts/run_complexity_compression_breakout.sh \
  --backtest-start-date 20260101 --backtest-end-date 20260331
./scripts/run_complexity_fractal_pullback.sh \
  --backtest-start-date 20260101 --backtest-end-date 20260331
./scripts/run_complexity_market_energy_flow.sh \
  --backtest-start-date 20260101 --backtest-end-date 20260331
./scripts/run_complexity_self_organized_trend.sh \
  --backtest-start-date 20260101 --backtest-end-date 20260331
...
| `entropy_bifurcation_setup/` | 熵分岔 4 层系统 & 复杂性子策略 |
| `four_layer_system/` | 分钟级四层系统 |
...
| `continuous_decline_recovery/` | 连续下跌恢复策略 |
| `volume_breakout/` | 量价突破策略 |
| `entropy_stock_picking/` | 熵选股 |
| `entropy_hold_judgement/` | 熵持有判断 |
| `entropy_hold_smoke/` | 熵持有冒烟测试 |
| `rapid_expansion_hold/` | 快速膨胀持有 |
| `rapid_expansion_exhaustion_exit/` | 衰竭退出 |
| `uptrend_hold_state_flow/` | 上升趋势状态流 |
| `stock_state_flow/` | 股票状态流 |
| `daily_backtest/` | 日线回测 |
| `factor_validation/` | 因子验证 |
| `strategy_runs/` | 临时策略运行 |
