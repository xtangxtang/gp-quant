...
DEFAULT_OUT_DIR="$WORKSPACE_DIR/results/entropy_bifurcation_setup/default_backtest_window"
DEFAULT_BACKTEST_END_DATE="$(date +%Y%m%d)"
DEFAULT_BACKTEST_START_DATE="$(date -d '120 days ago' +%Y%m%d)"
PYTHON_CMD=(
...
MIN_TURNOVER="1.0"
EXCLUDE_ST="1"
BACKTEST_START_DATE="$DEFAULT_BACKTEST_START_DATE"
BACKTEST_END_DATE="$DEFAULT_BACKTEST_END_DATE"
HOLD_DAYS="5"
MAX_POSITIONS="10"
...
show_help() {
  ...
  echo "  --out-dir <dir>          输出目录，默认写到 results/entropy_bifurcation_setup/default_backtest_window"
  ...
  echo "  --backtest-start-date <yyyymmdd>  前瞻回测起始日期，默认最近 120 天起点"
  echo "  --backtest-end-date <yyyymmdd>    前瞻回测结束日期，默认今天"
  echo "  --hold-days <n>          回测持有天数，默认 5"
  echo "  --max-positions <n>      组合最大持仓数，默认 10"
  echo "  --max-positions-per-industry <n>  每个行业最大持仓数，默认 2"
  echo "  -h, --help               显示帮助"
  echo ""
  echo "当前默认回测窗口: ${BACKTEST_START_DATE} .. ${BACKTEST_END_DATE}"
}
