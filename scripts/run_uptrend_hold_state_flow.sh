...
DEFAULT_OUT_DIR="$WORKSPACE_DIR/results/uptrend_hold_state_flow"
PYTHON_CMD=(
...
show_help() {
  ...
  echo "  --start-date <yyyymmdd>  买点日期，用于从该日开始评估持有路径"
  echo "  --data-dir <dir>         日线 CSV 目录"
  echo "  --out-dir <dir>          输出目录，默认写到 results/uptrend_hold_state_flow"
  echo "  --scan-date <yyyymmdd>   路径评估终点；默认自动推断最新交易日"
...
CMD=(
  src/strategy/uptrend_hold_state_flow/run_uptrend_hold_state_flow_scan.py
  --data_dir "$DATA_DIR"
  --out_dir "$OUT_DIR"
