...
**日期**: 2026-04-26  
**状态**: ⚠️ 假设 1 已证伪，见下方更新（2026-04-26 实验 A）
...
#### 1. 因子集不同（max_stocks 500 → 800）~~—— 已证伪~~ ⚠️

> **证伪（2026-04-26 实验 A）**: 实测 v1 (800股) 和 v2 (500股) 的 47 因子完全相同（字母序同一组）。`max_stocks` 只影响训练样本量，不影响因子列。IC=0.017 → 0.055 的改善**全部归因于正则化修复**（dropout 0.2→0.1, weight_decay 0.01→0.0001）。
...
当前 Group Embed IC=0.017 的结果**不能说明分组嵌入无效**。

---

## 后续更新（2026-04-26 实验 A）

### 假设验证结果

| 假设 | 状态 | 说明 |
|------|------|------|
| 1. 因子集不同 | ❌ **已证伪** | v1 和 v2 的 47 因子完全相同 |
| 2. 正则化配置错误 | ✅ **证实** | IC 下降全部归因于此 |
| 3. 冗余参数 | ✅ 已确认 | 不影响训练，但浪费参数量 |

### 实验 A 结论

控制变量后（max_stocks=500, dropout=0.1, wd=0.0001），分组嵌入 Eval IC = 0.055 vs Phase 1 Linear = 0.058 — **基本持平**。

- 分组嵌入增加了复杂度但无 IC 收益，已回退到 Phase 1 Linear 嵌入
- 详见 `20260426_05_cleanup_notes.md`
