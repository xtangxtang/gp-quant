...
|------|------|------|
| 总体准确率 | 46.8% | 与随机相当 |
| accumulation | 50.9% | 略优于随机，最佳状态 |
| collapse | 45.8% | 假阳性过多 |
| breakout | 47.1% | 样本太少 (17 次) |
| 累计验证数 | 4,200 | 持续积累中 |

### 问题诊断

1. **IC 值过低**: 5000+ 样本下单日截面 Spearman IC ~1e-5 量级，IC 对权重更新的贡献微乎其微
2. **collapse 阈值过松**: 45.8% 准确率，远低于 accumulation 的 50.9%
3. **breakout 阈值过严**: 15 期仅 17 次触发，hold 状态几乎不出现
4. **状态分布失衡**: 94.2% idle，有意义信号仅 ~6%

### 下一轮优化优先级

| 优先级 | 改动 | 预期效果 |
|--------|------|---------|
| P0 | 收紧 collapse 阈值 (collapse_need_n: 3→4) | 降低假阳性崩塌 |
| P0 | 放宽 breakout 阈值 | 增加突破信号数量 |
| P1 | 多日滚动 IC (20 日均值) | 提高权重更新稳定性 |
| P1 | 增加基本面因子 | 提升因子判别力 |
| P2 | 动态学习率 | 市场波动大时降速 |
| P2 | accumulation 持续时间约束 | 过滤瞬态假信号 |
