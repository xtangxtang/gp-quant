...
from .complexity_feature_engine import build_complexity_feature_frame
from .complexity_report_writer import write_complexity_outputs
from .complexity_signal_models import VALID_STRATEGIES, apply_strategy_model


@dataclass(frozen=True)
class ComplexityScanConfig:
    strategy_name: str
    data_dir: str
    out_dir: str
    scan_date: str = ""
...
def _prepare_symbol_state(file_path: str, config: ComplexityScanConfig, basic_info_map: dict[str, dict[str, str]]) -> PreparedSymbol | None:
    try:
    ...
    features = build_complexity_feature_frame(df)
    if features.empty:
        return None
...
def _prepare_all_symbols(files: list[str], config: ComplexityScanConfig, basic_info_map: dict[str, dict[str, str]]) -> list[PreparedSymbol]:
...
def _build_snapshot_row(prepared: PreparedSymbol, scan_date: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "ret_20": float(row["ret_20"]) if pd.notna(row.get("ret_20")) else None,
        "ret_60": float(row["ret_60"]) if pd.notna(row.get("ret_60")) else None,
        "breakout_20": float(row["breakout_20"]) if pd.notna(row.get("breakout_20")) else None,
        "pullback_depth_8": float(row["pullback_depth_8"]) if pd.notna(row.get("pullback_depth_8")) else None,
        "compression_z_120": float(row["compression_z_120"]) if pd.notna(row.get("compression_z_120")) else None,
    }


def _augment_cross_section(rows: list[dict[str, Any]], strategy_name: str) -> list[dict[str, Any]]:
    if not rows:
        return rows
    df = pd.DataFrame(rows)
    df["strategy_score"] = pd.to_numeric(df.get("strategy_score"), errors="coerce")
    df["energy_impulse"] = pd.to_numeric(df.get("energy_impulse"), errors="coerce")
    df["ret_20"] = pd.to_numeric(df.get("ret_20"), errors="coerce")
    df["breakout_20"] = pd.to_numeric(df.get("breakout_20"), errors="coerce")
    df["score_pct_rank"] = df["strategy_score"].rank(pct=True, method="average")
    df["energy_pct_rank"] = df["energy_impulse"].rank(pct=True, method="average")
    df["ret20_pct_rank"] = df["ret_20"].rank(pct=True, method="average")
    df["breakout_pct_rank"] = df["breakout_20"].rank(pct=True, method="average")

    if strategy_name == "market_energy_flow":
        energized = df["energy_impulse"].fillna(-999.0) >= 0.15
        industry_energy_count = df.loc[energized].groupby("industry")["symbol"].count().to_dict()
        df["industry_energy_count"] = df["industry"].map(industry_energy_count).fillna(0).astype(int)
        max_count = max(int(df["industry_energy_count"].max()), 1)
        df["industry_energy_rank"] = df["industry_energy_count"] / float(max_count)
    else:
        df["industry_energy_count"] = 0
        df["industry_energy_rank"] = 0.0

    return df.to_dict(orient="records")


def _passes_candidate_filters(row: dict[str, Any], config: ComplexityScanConfig) -> bool:
    ...
    if turnover_rate < float(config.min_turnover):
        return False
    if str(config.strategy_name) == "market_energy_flow":
        if float(row.get("score_pct_rank") or 0.0) < 0.90:
            return False
        if float(row.get("energy_pct_rank") or 0.0) < 0.85:
            return False
        if int(row.get("industry_energy_count") or 0) < 2:
            return False
    return True
...
def _sort_candidate_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted(
        ...
        key=lambda row: (
            ...
            float(row.get("score_pct_rank") or 0.0),
            float(row.get("energy_term") or -999.0),
...
def _select_candidate_rows(candidate_rows: list[dict[str, Any]], target_count: int, max_positions_per_industry: int) -> tuple[list[dict[str, Any]], int]:
...
def _build_scan_rows(prepared_symbols: list[PreparedSymbol], config: ComplexityScanConfig) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    market_rows = [row for row in (_build_snapshot_row(prepared, str(config.scan_date)) for prepared in prepared_symbols) if row]
    market_rows = _augment_cross_section(market_rows, config.strategy_name)
...
def _run_forward_backtest(prepared_symbols: list[PreparedSymbol], config: ComplexityScanConfig) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any] | None]:
    ...
    for scan_date in ordered_scan_dates:
        ...
        market_rows_day = [
            row
            for row in (_build_snapshot_row(prepared, scan_date) for prepared in prepared_symbols)
            if row is not None
        ]
        market_rows_day = _augment_cross_section(market_rows_day, config.strategy_name)
...
def run_complexity_scan(config: ComplexityScanConfig) -> list[str]:
    ...
    config = ComplexityScanConfig(**{**config.__dict__, "scan_date": effective_scan_date})
    ...
    return write_complexity_outputs(
