...
def evaluate_rapid_expansion_exhaustion_path(
    df: pd.DataFrame,
    start_date: str,
    scan_date: str = "",
    ...
    active_state = np.ones(len(out), dtype=bool)
    if bool(out["exit_persist"].any()):
        first_exit_idx = int(np.flatnonzero(out["exit_persist"].to_numpy(dtype=bool))[0])
        active_state[first_exit_idx:] = False
    out["hold_path_state"] = active_state
    out["strategy_state"] = active_state
    out["strategy_score"] = pd.to_numeric(out["exhaustion_exit_score"], errors="coerce").fillna(0.0)
    out["exit_score"] = out["strategy_score"]
    out["strategy_name"] = "rapid_expansion_exhaustion_exit"
    return out